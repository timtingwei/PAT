...
// 1071. 小赌怡情(15).cpp   3:18 -> 3:39 -> 3:46(update)
/*
...
int main() {
  ...
  while (T--) {
    ...
    if ((n1 < n2 && b) || (n1 > n2 && !b)) {
      ...
      if (!total) { cout << "Game Over." << endl; break;}
    }
  }
  return 0;
}
...
// 坑点2: 输了之后, 不用再判断是否剪完的total<0, 因为筹码不够早就continue,
//        只要判断total是否为0就行

// 输出的先后顺序, 首先要判断筹码是否足够, 接着判断是赢还是输, 赢没什么问题,
// 输了先打印剩余的Total, 根据Total来判断游戏是否结束
